...
scores = cross_val_score(model, attr, newlabels, cv=10, n_jobs=-1)
print("ADA-Boost: {}".format(scores.mean()))

# Bagging
model = sklearn.ensemble.BaggingClassifier(n_estimators=15)
scores = cross_val_score(model, attr, newlabels, cv=20, n_jobs=-1)
print("Bagging: {}".format(scores.mean()))

# Extra trees
model = sklearn.ensemble.ExtraTreesClassifier(n_estimators=10)
scores = cross_val_score(model, attr, newlabels, cv=20, n_jobs=-1)
